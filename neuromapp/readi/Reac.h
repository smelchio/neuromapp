...
public:
    ...
    // c-tor                                         // e.g. 2B + C --> A
    Reac(std::vector<std::string> const& spec_names, // e.g. {'A', 'B', 'C', 'D', 'E'}
         std::vector<std::string> const& lhs,        // e.g. {'B', 'B', 'C'}
         std::vector<std::string> const& rhs,        // e.g. {'A'}
         FloatType reaction_rate) :                  // the k_cst constant of reaction
        order_(lhs.size()),
        reaction_rate_(reaction_rate)
    {
        std::map<IntType, IntType> lhs_counts_map; // tells us that in LHS species idxed by 2 ("B") appears 2 times, species idxed by 3 appears 1 time.
        std::map<IntType, IntType> rhs_counts_map;

        // 1. count species in LHS
        for (auto& sp : lhs) {
            IntType idx = std::find(spec_names.begin(), spec_names.end(), sp) - spec_names.begin();
            if(idx==spec_names.size())
                throw std::range_error("species "+sp+" from lhs is undeclared!");
            else {
        ...
        for (auto& sp : rhs) {
            IntType idx = std::find(spec_names.begin(), spec_names.end(), sp) - spec_names.begin();
            if(idx==spec_names.size())
                throw std::range_error("species "+sp+" from rhs is undeclared!");
            else {
        ...
        // 3.1. create a set with all the idxes of the species that appear at least in one of the two maps
        std::set<IntType> lhs_rhs_idxes;
        for(auto& el : lhs_counts_map) lhs_rhs_idxes.insert(el.first);
        for(auto& el : rhs_counts_map) lhs_rhs_idxes.insert(el.first);
        // 3.2. for each idx in either rhs or lhs, check if it belongs to lhs only, rhs only, or both
        for (auto idx : lhs_rhs_idxes) {
            // if the idx is on both sides AND the difference is not 0, then add to update vector
            if (lhs_counts_map.count(idx) && rhs_counts_map.count(idx)) {
                if(lhs_counts_map[idx] != rhs_counts_map[idx]) {
                    upd_idxs_.push_back(idx);
                    upd_counts_.push_back(rhs_counts_map[idx]-lhs_counts_map[idx]);
                }
            }
            else {
    ...
    inline FloatType compute_c_mu(FloatType reaction_rate, FloatType volume) const {
    ...
    FloatType compute_propensity(IntType tet_idx, readi::Tets<IntType,FloatType>const& tets) const {
        FloatType propensity = compute_c_mu(reaction_rate_, tets.volume(tet_idx));
    ...
    inline std::vector<IntType> get_lhs_idxs() const {
        return lhs_idxs_;
    }

    inline std::vector<IntType> get_lhs_counts() const {
        return lhs_counts_;
    }

    inline std::vector<IntType> get_rhs_idxs() const {
        return rhs_idxs_;
    }

    inline std::vector<IntType> get_rhs_counts() const {
        return rhs_counts_;
    }

    inline std::vector<IntType> get_upd_idxs() const {
        return upd_idxs_;
    }

    inline std::vector<IntType> get_upd_counts() const {
        return upd_counts_;
    }

    inline FloatType get_rate() const {
...
private:
    ...
    std::vector<IntType> upd_counts_;       // counts of molecules per species in update vector
